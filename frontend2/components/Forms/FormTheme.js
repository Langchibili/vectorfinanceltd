...
//  The light-mode rules used to live inside
//  `@media (prefers-color-scheme: light)`, which reads the OS-level
//  setting and has nothing to do with the app's own toggle. They've
//  been moved to `[data-theme="light"]`, which is the attribute
//  ThemeProvider.jsx actually sets on <html> — so these forms now
//  follow the in-app switch instead of the OS.
//
//  The JS style-object exports below (G, btnPrimary, cardStyle,
//  inputStyle, labelStyle, dividerStyle, alertConfig) are plain
//  objects evaluated once at import time — they can't react to a
//  runtime toggle on their own. Light-mode counterparts (GL,
//  cardStyleLight, inputStyleLight, labelStyleLight, dividerStyleLight,
//  btnOutlineLight, alertConfigLight) are added below so any component
//  using these can pick the right one with `isDark ? cardStyle : cardStyleLight`
//  via `useThemeMode()`. The original exports are untouched, so nothing
//  that currently imports them breaks.
// ═══════════════════════════════════════════════════════════════
...
// ── light-mode counterpart palette ─────────────────────────────
export const GL = {
    page: "#F7F5F0",
    card: "#FFFFFF",
    cardHov: "#FFFFFF",
    ...
    purple: "#7C3AED",
    white: "#0A1628",
    muted: "rgba(10,22,40,0.50)",
    dim: "rgba(10,22,40,0.04)",
    border: "rgba(10,22,40,0.12)",
    borderG: "rgba(5,150,105,0.25)",
    inputBg: "rgba(10,22,40,0.04)",
    inputBgHov: "rgba(5,150,105,0.05)",
}

// ── button base styles (brand gradients read fine on both themes) ──
export const btnPrimary = {
    display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7,
    background: "linear-gradient(135deg,#059669 0%,#10B981 55%,#059669 100%)",
    backgroundSize: "200% auto",
...
export const cardStyleLight = {
    ...
    borderRadius: 20,
    boxShadow: "0 8px 40px rgba(0,0,0,0.08), 0 1px 0 rgba(16,185,129,0.10) inset",
    overflow: "hidden",
    backdropFilter: "blur(16px)",
    WebkitBackdropFilter: "blur(16px)",
}
...
export const dividerStyleLight = {
    ...dividerStyle,
    background: "linear-gradient(90deg, transparent, rgba(5,150,105,0.20), transparent)",
}
...
export const alertConfigLight = {
    success: { bg: "rgba(16,185,129,0.08)", border: "rgba(16,185,129,0.25)", color: "#047857", glow: "rgba(16,185,129,0.04)", icon: "✓" },
    info: { bg: "rgba(37,99,235,0.08)", border: "rgba(37,99,235,0.22)", color: "#1D4ED8", glow: "rgba(37,99,235,0.04)", icon: "ℹ" },
    warning: { bg: "rgba(217,119,6,0.08)", border: "rgba(217,119,6,0.22)", color: "#B45309", glow: "rgba(217,119,6,0.04)", icon: "⚠" },
    error: { bg: "rgba(220,38,38,0.08)", border: "rgba(220,38,38,0.22)", color: "#B91C1C", glow: "rgba(220,38,38,0.04)", icon: "✕" },
}
...
export const FORM_CSS = `
  ...
  /* ── light mode overrides ─────────────────────────────────
     Was: @media (prefers-color-scheme: light) — keyed off the OS
     setting. Now keyed off [data-theme="light"], the attribute the
     app's own toggle actually sets on <html>, so these forms follow
     the in-app switch. Since the app now defaults to light, these
     rules apply out of the box until someone flips to dark. ── */
  [data-theme="light"] .vf-card {
    background: linear-gradient(160deg, rgba(5,150,105,0.05) 0%, #ffffff 40%, #f8fffe 100%);
    border-color: rgba(5,150,105,0.20);
    box-shadow: 0 8px 40px rgba(0,0,0,0.08), inset 0 1px 0 rgba(16,185,129,0.10);
  }
  [data-theme="light"] .vf-card-header {
    background: linear-gradient(180deg, rgba(5,150,105,0.05) 0%, transparent 100%);
  }
  [data-theme="light"] .vf-card-title { color: #0A1628; }
  [data-theme="light"] .vf-card-subtitle { color: rgba(10,22,40,0.50); }
  [data-theme="light"] .vf-label { color: rgba(10,22,40,0.50); }
  [data-theme="light"] .vf-sublabel { color: rgba(10,22,40,0.38); }
  [data-theme="light"] .vf-input,
  [data-theme="light"] .vf-select,
  [data-theme="light"] .vf-textarea {
    background: rgba(10,22,40,0.04);
    border-color: rgba(10,22,40,0.14);
    color: #0A1628;
  }
  [data-theme="light"] .vf-input::placeholder,
  [data-theme="light"] .vf-textarea::placeholder {
    color: rgba(10,22,40,0.30);
  }
  [data-theme="light"] .vf-input:hover,
  [data-theme="light"] .vf-select:hover,
  [data-theme="light"] .vf-textarea:hover {
    border-color: rgba(5,150,105,0.35);
    background: rgba(5,150,105,0.04);
  }
  [data-theme="light"] .vf-input:focus,
  [data-theme="light"] .vf-select:focus,
  [data-theme="light"] .vf-textarea:focus {
    border-color: rgba(5,150,105,0.60);
    background: rgba(5,150,105,0.05);
    box-shadow: 0 0 0 3px rgba(5,150,105,0.10);
    color: #0A1628;
  }
  [data-theme="light"] .vf-select option { background: #fff; color: #0A1628; }
  [data-theme="light"] .vf-input-prefix,
  [data-theme="light"] .vf-input-suffix {
    background: rgba(5,150,105,0.07);
    border-color: rgba(5,150,105,0.22);
    color: #059669;
  }
  [data-theme="light"] .vf-radio-label {
    color: rgba(10,22,40,0.70);
    border-color: rgba(10,22,40,0.12);
    background: rgba(10,22,40,0.03);
  }
  [data-theme="light"] .vf-radio-label:hover {
    border-color: rgba(5,150,105,0.35);
    background: rgba(5,150,105,0.05);
    color: #0A1628;
  }
  [data-theme="light"] .vf-radio-label.active { color: #059669; }
  [data-theme="light"] .vf-section-title h5,
  [data-theme="light"] .vf-section-title h4,
  [data-theme="light"] .vf-section-title h6 {
    color: rgba(10,22,40,0.50);
  }
  [data-theme="light"] .vf-divider {
    background: linear-gradient(90deg, transparent, rgba(5,150,105,0.20), transparent);
  }
  [data-theme="light"] .vf-summary {
    background: linear-gradient(135deg, rgba(5,150,105,0.06), rgba(5,150,105,0.02));
    border-color: rgba(5,150,105,0.18);
  }
  [data-theme="light"] .vf-summary-label { color: rgba(10,22,40,0.45); }
  [data-theme="light"] .vf-summary-value { color: #059669; }
  [data-theme="light"] .vf-page { background: #f0fdf8; }
  [data-theme="light"] .vf-page::before {
    background:
      radial-gradient(ellipse 90% 50% at 50% -5%, rgba(5,150,105,0.07) 0%, transparent 65%),
      radial-gradient(ellipse 60% 40% at 85% 85%, rgba(5,150,105,0.05) 0%, transparent 60%);
  }
  [data-theme="light"] .vf-btn-outline { color: #059669; border-color: rgba(5,150,105,0.38) !important; }
  [data-theme="light"] .vf-btn-outline:hover:not(:disabled) {
    background: rgba(5,150,105,0.08);
    border-color: rgba(5,150,105,0.55) !important;
  }
  [data-theme="light"] .vf-summary-row { border-bottom-color: rgba(10,22,40,0.06); }
`