...
export default class UploadSessionLetterTemplate extends React.Component {
  state = { open: false, loading: false, error: '', sessionLetterTemplate: null }

  open = () => {
    const { loan } = this.props
    let sessionLetterTemplate = loan?.collateral?.vehicle?.sessionLetterTemplate ?? null
    if (sessionLetterTemplate && sessionLetterTemplate.data) sessionLetterTemplate = sessionLetterTemplate.data
    this.setState({ open: true, error: '', sessionLetterTemplate })
  }
  ...
  addSessionLetterTemplate = async (uploaded) => {
    ...
    if (sessionLetterTemplate && sessionLetterTemplate.data) {
      const data = Array.isArray(sessionLetterTemplate.data) ? sessionLetterTemplate.data[0] : sessionLetterTemplate.data
      ...
      if (t.data) { const data = Array.isArray(t.data) ? t.data[0] : t.data; existing = !!(data && (data.id || data.url)) }
      else { existing = !!(t.id || t.url) }
    }
    if (existing) {
      const ok = window.confirm('A session letter template is already uploaded. Do you want to replace it?')
      if (!ok) return
    }
    ...
    try {
      const body = {
        data: {
          ...
          actionPayload: { sessionLetterTemplate: uploadedId ?? uploadedRaw },
  ...
  render() {
    ...
    if (sessionLetterTemplate && sessionLetterTemplate.data) {
      ...
      const t = loan.collateral.vehicle.sessionLetterTemplate
      if (t.data) { const data = Array.isArray(t.data) ? t.data[0] : t.data; hasSessionLetterTemplate = !!(data && (data.id || data.url)) }
      else { hasSessionLetterTemplate = !!(t.id || t.url) }
    }
      ...
      <ThemeProvider theme={adminTheme}>
        ...
        <Dialog open={open} onClose={this.close} fullWidth maxWidth="sm">
          ...
          <DialogContent>
            ...
            {hasSessionLetterTemplate && (
              <Alert severity="warning" sx={{ mb: 2 }}>
                A session letter template is already uploaded. Uploading a new file will replace it.
              </Alert>
            )}

            <Box sx={{ mb: 2, p: '16px', background: G.dim, border: `1px solid ${G.border}`, borderRadius: '12px' }}>
              <Uploader
            ...
            {loading && (
              <Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 1 }}>
                ...
                <Typography variant="body2" sx={{ color: G.muted }}>Uploading session letter template…</Typography>
              </Stack>
            )}
          </DialogContent>
          ...
          <DialogActions>
            ...
            <Button
              onClick={() => {
                if (!hasSessionLetterTemplate) { alert('Please choose a file to upload using the uploader above'); return }
