...
export default function CountrySelect({ setCountry }) {
  const [selectedCountry, setSelectedCountry] = useState('Zambia') // Default to Zambia

  const handleCountryChange = (event) => {
    const newCountry = event.target.value
    setSelectedCountry(newCountry)
    setCountry(newCountry) // Call the provided prop function
  }

  useEffect(() => { // Set the default country to Zambia
    setCountry('Zambia') // Call the provided prop function
  }, [])

  return (
    <Box sx={{ width: "100%", marginBottom: '8px' }}>
      <Typography variant="h6" sx={{ fontWeight: 'light' }} gutterBottom align='left'>
        Select Country
      </Typography>
        ...
        <FormControl fullWidth>
          ...
          >
            {countries.map((country, index) => (
              <MenuItem key={`${country.name}-${index}`} value={country.name}>
                {country.name}
              </MenuItem>
            ))}
          </Select>
